...
from Applications.inpainting.lama.common import OfflineInpainter

class LamaMPEInpainter(OfflineInpainter):
    _MODEL_MAPPING = {
        'model': {
            'url': 'https://drive.google.com/file/d/1Qvop0I9_p6EnbZTJO9A_LLUEgwUkZ4T8/view?usp=sharing',
    ...
    def resize_keep_aspect(self, img, size):
    ...
    async def _infer(self, image: np.ndarray, mask: np.ndarray, inpainting_size: int = 1024, verbose: bool = False) -> np.ndarray:
        ...
        if max(image.shape[0: 2]) > inpainting_size:
            image = self.resize_keep_aspect(image, inpainting_size)
            mask =  self.resize_keep_aspect(mask, inpainting_size)
        pad_size = 8
        h, w, c = image.shape
        if h % pad_size != 0:
